resource "datadog_monitor" "apm_service_high_latency" {
  ...
  name               = "Service ${each.key} has a high latency on ${each.value.environment}"
  type               = "query alert"
  ...
  query = "avg(last_10m):avg:trace.aws.${each.key}{env:none,service:${each.key}} > ${each.value.high_error_rate_critical}"
  thresholds = {
